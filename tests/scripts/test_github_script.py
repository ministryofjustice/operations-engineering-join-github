...
class TestGithubScript(unittest.TestCase):
    ...
    def test_is_email_address_pre_approved_with_as_org_approved_email_addresses(self, mock_github_service):
        ...
        for email_domain in AS_ORG_ALLOWED_EMAIL_DOMAINS:
            ...
            is_pre_approved = github_script._is_email_address_pre_approved(MOJ_ANALYTICAL_SERVICES, email_address)
            self.assertTrue(is_pre_approved)

    @patch("landing_page_app.main.services.github_service")
    def test_is_email_address_pre_approved_with_moj_org_approved_email_addresses(self, mock_github_service):
        github_script = GithubScript(mock_github_service)
        for email_domain in MOJ_ORG_ALLOWED_EMAIL_DOMAINS:
            email_address = f"{self.test_user}@{email_domain}"
            is_pre_approved = github_script._is_email_address_pre_approved(MINISTRY_OF_JUSTICE, email_address)
            self.assertTrue(is_pre_approved)

    @patch("landing_page_app.main.services.github_service")
    def test_is_email_address_pre_approved_with_non_approved_email_address(self, mock_github_service):
        github_script = GithubScript(mock_github_service)
        is_pre_approved = github_script._is_email_address_pre_approved(MOJ_TEST_ORG, "some@email.com")
        self.assertFalse(is_pre_approved)

    @patch("landing_page_app.main.services.github_service")
    def test_is_email_address_pre_approved_with_incorrect_inputs(self, mock_github_service):
        github_script = GithubScript(mock_github_service)
        is_pre_approved = github_script._is_email_address_pre_approved(MOJ_TEST_ORG, self.test_email_address)
        self.assertFalse(is_pre_approved)

        is_pre_approved = github_script._is_email_address_pre_approved(MOJ_TEST_ORG, None)
        self.assertFalse(is_pre_approved)

        is_pre_approved = github_script._is_email_address_pre_approved(MOJ_TEST_ORG, "")
        self.assertFalse(is_pre_approved)

        is_pre_approved = github_script._is_email_address_pre_approved(None, self.test_email_address)
        self.assertFalse(is_pre_approved)

        is_pre_approved = github_script._is_email_address_pre_approved("", self.test_email_address)
        self.assertFalse(is_pre_approved)

    @patch("landing_page_app.main.services.github_service")
    def test_add_new_user_to_github_org_with_approved_email_addresses(self, mock_github_service):
    ...
    @patch("landing_page_app.main.services.github_service")
    def test_add_new_user_to_github_org_with_non_approved_email_addresses(self, mock_github_service):
    ...
    @patch("landing_page_app.main.services.github_service")
    def test_add_new_user_to_github_org_with_incorrect_inputs(self, mock_github_service):
        github_script = GithubScript(mock_github_service)
        ...
        github_script.add_new_user_to_github_org(self.test_user, self.test_email_address, [])
        github_script.github_service.get_user.assert_not_called()

        github_script.add_new_user_to_github_org("", self.test_email_address, [MOJ_TEST_ORG])
        github_script.github_service.get_user.assert_not_called()

        github_script.add_new_user_to_github_org(None, self.test_email_address, [MOJ_TEST_ORG])
        github_script.github_service.get_user.assert_not_called()
