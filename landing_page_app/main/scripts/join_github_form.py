...
class JoinGithubForm(Form):
    gh_username = StringField("GitHub Username", [validators.Length(min=3, max=25),
                                                  InputCheck(regex="^(?=.*[+_!@#$%^&*., ?])", message="Special characters not allowed"),
                                                  validators.InputRequired()])
    email_address = StringField("Email Address", [validators.Length(min=6, max=120),
                                                  InputCheck(regex="^(?=.*[-+!#$%^&*,?])", message="Special characters not allowed"),
    ...
    def validate_org(self):
        if self.access_moj_org.data is False and self.access_as_org.data is False or self.access_moj_org.data is None and self.access_as_org.data is None:
